...
func (d *Database) Query(ctx context.Context, query string, args ...interface{}) (Rows, error) {
	if d == nil || d.pool == nil {
...
func (d *Database) QueryRow(ctx context.Context, query string, args ...interface{}) Row {
	if d == nil || d.pool == nil {
		db, host, port, user := d.getDBInfo()
...
func (d *Database) Exec(ctx context.Context, query string, args ...interface{}) (CommandResult, error) {
	if d == nil || d.pool == nil {
		db, host, port, user := d.getDBInfo()
		return nil, fmt.Errorf("database connection not established: database '%s' on host '%s:%d' as user '%s' (connection pool is nil, ensure Connect() was called successfully)", db, host, port, user)
	}
