...
/* Annotations returns MCP tool hints (readOnlyHint, destructiveHint, etc.) */
func (b *BaseTool) Annotations() ToolAnnotations {
	if b == nil {
...
/* SetAnnotations sets the tool annotations */
func (b *BaseTool) SetAnnotations(a ToolAnnotations) {
	if b != nil {
		b.annotations = a
	}
}
...
/* validatePattern validates a string against a JSON Schema pattern (ECMA 262 style regex).
 * Uses Go regexp (RE2) for safety. Patterns are anchored to full string match.
 * Max pattern length 1024 to reduce ReDoS risk.
 */
...
func validatePattern(value, pattern string) (bool, error) {
	...
	if len(pattern) > maxPatternLength {
		return false, fmt.Errorf("pattern length %d exceeds maximum %d", len(pattern), maxPatternLength)
	}
