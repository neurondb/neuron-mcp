...
func TestValidatePattern(t *testing.T) {
	tests := []struct {
		...
		{"", "", true, false},
		{"hello", "^hel.o$", true, false},
		{"hello", "^world$", false, false},
		{"abc123", `^[a-z]+\d+$`, true, false},
		{"x", "x", true, false},
		{"invalid(regex", "(", false, true},
	}
	for _, tt := range tests {
		got, err := validatePattern(tt.value, tt.pattern)
		if (err != nil) != tt.wantErr {
			t.Errorf("validatePattern(%q, %q) err = %v, wantErr %v", tt.value, tt.pattern, err, tt.wantErr)
			continue
		}
		if !tt.wantErr && got != tt.want {
			t.Errorf("validatePattern(%q, %q) = %v, want %v", tt.value, tt.pattern, got, tt.want)
		}
	}
	/* Pattern too long */
	_, err := validatePattern("x", string(make([]byte, maxPatternLength+1)))
	if err == nil {
		t.Error("validatePattern with too long pattern should return error")
	}
}
