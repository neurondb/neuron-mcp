...
/* Transport is the interface for MCP message transport (stdio, HTTP, HTTPS) */
type Transport interface {
	ReadMessage() (*JSONRPCRequest, error)
	WriteMessage(resp *JSONRPCResponse) error
	Close() error
}
...
/* Close closes the transport (no-op for stdio; implements Transport interface) */
